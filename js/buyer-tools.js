(() => {
  ...
  form.addEventListener('submit', (event) => {
    ...
    const message = `Hello, I would like a quotation. Product: Roto Water Tank. Capacity: ${recommended.capacity.toLocaleString()}L. Quantity: 1. Delivery location: [county/town]. The tank wizard estimated ${Math.round(calculated).toLocaleString()} litres for a ${property} serving ${people} people for ${days} backup days. Please also quote the ${alternative.capacity.toLocaleString()}L alternative and confirm delivery cost.`;
    result.hidden = false;
